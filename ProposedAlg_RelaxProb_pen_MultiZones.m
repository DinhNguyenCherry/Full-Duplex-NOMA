...
function [ isBreak, global_OptValue, global_OptValueChain, global_ULRate_PerUser, global_DLRate_PerUser, global_Solution ] = ProposedAlg_RelaxProb_pen_MultiZones( Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, strdisp )
%PROPOSEDALG Summary of this function goes here
%   Detailed explanation goes here

if (nargin==nargin('ProposedAlg_RelaxProb_pen')-1)
    strdisp = [];
end
...
Converg = findstr(strdisp,'Convergence');
if (Converg)
    if (strcmp(strdisp,'Convergence2'))
        MaxIteration = 5;
    else
        MaxIteration = 50;
    end
...
global_OptValue = -1000;
        ...
        while (GetBreak)
            [ GetBreak, OptimalValue, OptimalValue_preStep, ~, ~, W_current, p_current, addVar_current, Chain] = ...
                    GetInitialization(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, {1, 'DL/PEN'}, {}, 1);
                addVar_current{1,1}
            RowSum = sum(round(addVar_current{1,1}(1:K,:)),1)
            ColSum = sum(round(addVar_current{1,1}(1:K,:)),2)
            
            RowSum2 = sum(round(addVar_current{1,1}(K+1:2*K,:)),1)
            ColSum2 = sum(round(addVar_current{1,1}(K+1:2*K,:)),2)
            
            RowSum3 = sum(round(addVar_current{1,1}(2*K+1:3*K,:)),1)
            ColSum3 = sum(round(addVar_current{1,1}(2*K+1:3*K,:)),2)
            
            if (~isempty(find(RowSum~=1)) || ~isempty(find(ColSum~=1)))
                GetBreak = 1;
            end
            
            if (~isempty(find(RowSum2~=1)) || ~isempty(find(ColSum2~=1)))
                GetBreak = 1;
            end
            
            if (~isempty(find(RowSum3~=1)) || ~isempty(find(ColSum3~=1)))
                GetBreak = 1;
            end
%             RowSum = sum(round(addVar_current{2}),1);
%             ColSum = sum(round(addVar_current{2}),2);
%             if (~isempty(find(RowSum<1)) || ~isempty(find(ColSum<1)))
%                 GetBreak = 1;
%             end
        end
        if (strcmp(strdisp,'Convergence2'))
            PenChain_alpha = Chain{1};
        end
        GetBreak = 1;
        while (GetBreak)
            [ GetBreak, OptimalValue, OptimalValue_preStep, ~, ~, W_current, p_current, addVar_current] = ...
                    GetInitialization(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, {1, 'UL/PEN'}, {W_current, p_current, addVar_current},1);
            GetBreak = 0;
            sum_beta = round(sum(addVar_current{1,2},2));
            for iOr = 0:1:L-1
                if (isempty(find(iOr==sum_beta)))
                    GetBreak = 1;
                    break
                end
            end
        ...
        if (~GetBreak)
            ...
            while (GetBreak)
                ...
                if (nInit==3)
                  ...
                  [ GetBreak, OptimalValue, OptimalValue_preStep, ~, ~, W_current, p_current, addVar_current] = ...
                    GetInitialization(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, {2, 'DL/UL'}, {W_current, p_current, addVar_current},1);
            end        
        end
        ...
        if (GetBreak)
            ...
            DownlinkRate_PerUser = zeros(K,3);
            
            global_OptValue = 0;
            
            global_ULRate_PerUser = zeros(L,1);
            global_DLRate_PerUser = zeros(K,3);
            ...
            UplinkRate_PerUser = 0;
            DownlinkRate_PerUser = 0;
...
%             end
            ...
            while (n<MaxIteration)
                ...
                Get_optSolutionPerIteration3_MultiZones(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, W_current, p_current, addVar_current, {0, 'DL/UL'}, max_min, eta, n);
                                                
                % Update
                
                
                if (isFeasible(Status, addVar_next, OptimalValue_current, OptimalValue, sum(UplinkRate_PerUser_next/log(2),2), sum(DownlinkRate_PerUser_next/log(2),2), Rate_Threshold, n, W_current, W_next))
...
%             global_UplinkPower_PerUser = sum(global_UplinkPower_PerGroupPerUser,2);
            if (strcmp(strdisp,'Convergence2'))
                global_OptValueChain = {PenChain_alpha, PenChain_beta};
            else
...
function [check] = isFeasible(Status, addVar_next, OptimalValue_current, OptimalValue, UplinkRate, DownlinkRate, Threshold, Step, W_current, W_next)
