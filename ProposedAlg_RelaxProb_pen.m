...
function [ isBreak, global_OptValue, global_OptValueChain, global_ULRate_PerUser, global_DLRate_PerUser, global_Solution ] = ProposedAlg_RelaxProb_pen( Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, strdisp )
...
global_OptValue = -1000;
        ...
        while (GetBreak)
            [ GetBreak, OptimalValue, OptimalValue_preStep, ~, ~, W_current, p_current, addVar_current, Chain] = ...
                    GetInitialization(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, {1, 'DL/PEN'}, {});
...
%             end
        ...
        while (GetBreak)
            [ GetBreak, OptimalValue, OptimalValue_preStep, ~, ~, W_current, p_current, addVar_current] = ...
                    GetInitialization(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, max_min, eta, {1, 'UL/PEN'}, {W_current, p_current, addVar_current});
            GetBreak = 0;
            sum_beta = round(sum(addVar_current{2},2));
            for iOr = 0:1:L-1
...
%             end
            ...
            while (n<MaxIteration)
                ...
                Get_optSolutionPerIteration3(Pbs, P, Hu, Hd, G_SI, G_lk, rho, Rate_Threshold, W_current, p_current, addVar_current, {0, 'DL/UL'}, max_min, eta, n);
...
if (length(OptValueChain)>10)
    ...
    if (~Converg)
    if (abs(OptimalValue-OptValueChain(length(OptValueChain)))/abs(OptimalValue) < 0.1)
        check = 1;
    end
    end
end
