function [ D_H1k, D_H2j, D_Hu, D_gCCI, positionDownlinkUsers, positionUplinkUsers ] = CreateD( K, L, Parameters, AllCells, Order, posDLUs, posULUs )
%CREATED Summary of this function goes here
%   Detailed explanation goes here

RadiusOfCell = Parameters(1);
RadiusOfNearestUser = Parameters(2);
InnerZoneRadius = Parameters(3);

if (nargin<nargin('CreateD'))
...
%% large-fading between uplink and downlink users
    ...
    positionDownUsers_Ltimes = kron(positionDownlinkUsers(1:2*K,:),ones(L,1));
    positionUpUsers_2Ktimes = repmat(positionUplinkUsers(1:L,:), 2*K, 1);
    ...
    D_gCCI = (reshape(PL_PQup2KLdown, L, 2*K)).^(0.5)
